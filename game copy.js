...
function setupTouchControls() {
    ...
    const buttons = [
        { id: 'left', x: startX, y: canvas.height - buttonHeight - bottomMargin, width: buttonWidth, height: buttonHeight, key: 'ArrowLeft', text: '←' },
        { id: 'jump', x: startX + buttonWidth + buttonSpacing, y: canvas.height - buttonHeight - bottomMargin, width: buttonWidth, height: buttonHeight, key: 'ArrowUp', text: '↑' },
        { id: 'right', x: startX + 2 * (buttonWidth + buttonSpacing), y: canvas.height - buttonHeight - bottomMargin, width: buttonWidth, height: buttonHeight, key: 'ArrowRight', text: '→' }
    ];
    ...
    // Draw buttons in the draw function
    function drawButtons() {
        if (!isMobileDevice() && !showControlsOnPC) return;
        ctx.save();
        ctx.font = '30px "Segoe UI Symbol", Arial, sans-serif'; // Use font stack with Unicode support
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        buttons.forEach(button => {
            // Draw button background
            ctx.fillStyle = keys.has(button.key) ? '#080816c5' : '#080816ff'; // Darker when pressed
